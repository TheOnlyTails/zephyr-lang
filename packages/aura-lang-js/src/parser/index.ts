...
class AuraParser extends CstParser {
	constructor() {
		super(allTokens)
	...
	public Program = this.RULE("Program", () => {
			this.MANY_SEP({
				SEP: this.OR([{ ALT: () => tokens.NEWLINE }, { ALT: () => tokens.SEMICOLON }]),
	...
	public Expression = this.RULE("Expression", () => {
		this.OR([
			...
			{ ALT: () => this.SUBRULE(this.Tuple) },
			{ ALT: () => this.SUBRULE(this.Object) },
			{ ALT: () => this.SUBRULE(this.Closure) },
			{ ALT: () => this.SUBRULE(this.Identifier) },
			{ ALT: () => this.SUBRULE(this.ClosureCall) },
			{ ALT: () => this.SUBRULE(this.BinaryOp) },
			{ ALT: () => this.SUBRULE(this.UnaryOp) },
			{ ALT: () => this.SUBRULE(this.Let) },
			{ ALT: () => this.SUBRULE(this.If) },
			{ ALT: () => this.SUBRULE(this.While) },
			{ ALT: () => this.SUBRULE(this.For) },
			{ ALT: () => this.SUBRULE(this.Import) },
			{ ALT: () => this.SUBRULE(this.Cast) },
			{ ALT: () => this.SUBRULE(this.TypeCheck) },
			{ ALT: () => this.SUBRULE(this.CodeBlock) },
			{ ALT: () => this.SUBRULE(this.Parens) },
		])
	})

	private Int = this.RULE("Int", () => this.CONSUME(tokens.INT_LITERAL))
	private Float = this.RULE("Float", () => this.CONSUME(tokens.FLOAT_LITERAL))
	private Boolean = this.RULE("Boolean", () => this.CONSUME(tokens.BOOLEAN_LITERAL))
	private Char = this.RULE("Char", () => this.CONSUME(tokens.CHAR_LITERAL))
	private String = this.RULE("String", () => this.CONSUME(tokens.STRING_LITERAL))
	private List = this.RULE("List", () => {
		this.CONSUME(tokens.LIST_START)
		this.MANY_SEP({
			SEP: tokens.COMMA,
			DEF: () => this.SUBRULE(this.Expression)
		})
	...
	private Tuple = this.RULE("Tuple", () => {
		this.CONSUME(tokens.TUPLE_START)
		this.MANY_SEP({
			SEP: tokens.COMMA,
			DEF: () => this.SUBRULE(this.Expression)
		})
		this.CONSUME(tokens.RIGHT_PAREN)
	})
	private Object = this.RULE("Object", () => {
		this.CONSUME(tokens.OBJECT_START)
		this.MANY_SEP({
			SEP: tokens.COMMA,
			DEF: () => {
	...
	private Closure = this.RULE("Closure", () => {
		this.CONSUME(tokens.LEFT_PAREN)
		this.MANY_SEP({
			SEP: tokens.COMMA,
			DEF: () => {
				this.CONSUME(tokens.IDENTIFIER)
	...
	private Type = this.RULE("Type", () => {
		this.OR([
			{ ALT: () => this.CONSUME(tokens.INT_TYPE) },
			{ ALT: () => this.CONSUME(tokens.FLOAT_TYPE) },
			{ ALT: () => this.CONSUME(tokens.CHAR_TYPE) },
			{ ALT: () => this.CONSUME(tokens.STRING_TYPE) },
			{ ALT: () => this.CONSUME(tokens.BOOLEAN_TYPE) },
			{ ALT: () => this.CONSUME(tokens.VOID_TYPE) },
			{
				/**
				 * List type
				 */
			...
			},
			{
				...
				ALT: () => {
					this.CONSUME(tokens.IDENTIFIER)
					this.OPTION(() => {
						this.CONSUME(tokens.LEFT_ANGLED)
						this.AT_LEAST_ONE_SEP({
							SEP: tokens.COMMA,
							DEF: () => this.SUBRULE(this.Type)
						})
						this.CONSUME(tokens.RIGHT_ANGLED)
					})
				}
			},
			{
